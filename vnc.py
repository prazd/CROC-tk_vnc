...
check = subprocess.check_output('''ps ax|grep 5900|grep -v
                                grep|awk \'{print $1}\' ''',
                                shell=True)
proc = str(check)
proc = proc.replace('\'', '')
...
def mes():
    messagebox.showinfo('info', '''Вы можете подключиться
                            по vnc 2мя способами:\n1.
                            Ввести номер хоста\n2.Ввести ip(имя) полностью\n
                            По умолчанию тунель пробрасывается на\n
                            localhost:5900\nПри подключении
                            можно указать порт самостоятельно''')
            ...
            if len(p) == 0:
                    try:
                        subprocess.call('''ssh -o StrictHostKeyChecking=no
                            -fN -L 5900:localhost:5900 host''' + q, shell=True)
                        subprocess.call('''vncviewer -passwordfile
                            /home/.vnc/passwd localhost ''', shell=True)
                        check = subprocess.check_output('''ps ax | grep
                             5900 |grep -v grep|awk
                             \'{print $1}\'''', shell=True)
                        proc = str(check)
                        proc = proc.replace('\'', '')
                        proc = proc.replace('b', '')
                        ...
                        res = [int(x) for x in res]
                            ...
                            if res:
                                ...
                                try:
                                        ...
                                        -9 ''' + str(res), shell=True)
                                except subprocess.CalledProcessError:
                                        print('BAD')
            else:
                    try:
                        subprocess.call('''ssh -o StrictHostKeyChecking=no
                            -fN -L '''+p+''':localhost:5900
                            host''' + q, shell=True)
                        subprocess.call('''vncviewer -passwordfile
                            /home/.vnc/passwd localhost:'''+p, shell=True)
                        check = subprocess.check_output('''ps ax | grep
                            '''+p+''' |grep -v grep|
...
def var2():
    def c():
        ...
        if len(p) == 0:
                    try:
                        subprocess.call('''ssh -o StrictHostKeyChecking=no
                        -fN -L 5900:localhost:5900 ''' + w, shell=True)
                        subprocess.call('''vncviewer -passwordfile
                        /home/.vnc/passwd localhost ''', shell=True)
                        check = subprocess.check_output('''ps ax |
                        grep 5900 |grep -v grep|awk
                        \'{print $1}\'''', shell=True)
                        proc = str(check)
                        proc = proc.replace('\'', '')
                        proc = proc.replace('b', '')
                    ...
                    try:
                        subprocess.call('''ssh -o
                            StrictHostKeyChecking=no
                            -fN -L '+p+':localhost:5900 ''' + w, shell=True)
                        subprocess.call('''vncviewer -passwordfile
                            /home/.vnc/passwd localhost:'''+p, shell=True)
                        check = subprocess.check_output('''ps ax | grep
                            '''+p+''' |grep -v grep|
